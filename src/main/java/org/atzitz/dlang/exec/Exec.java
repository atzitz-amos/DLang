...
public class Exec {
    ...
    void step() {
        ...
        switch (bc.type) {
            ...
            case AbstractBytecode.Type.LoadAttr -> execLoadAttr((BCLoadDynamic) bc);
            ...
            case AbstractBytecode.Type.StoreAttr -> execStoreAttr((BCStoreAttr) bc);
            ...
            case AbstractBytecode.Type.CallFunc -> execFuncCall((BCCallFunc) bc);
    ...
    private void execFuncCall(BCCallFunc bc) {
        int jumpTo = env.mem.get(bc.id);

        env.stack.push(env.mem.$LOCAL);
        env.stack.push(env.mem.$PARAM);
        env.stack.push(env.mem.$THIS);
        env.stack.push(env.pc.value);

        env.mem.$PARAM = env.mem.$SP - 4 - bc.argc;
    ...
    private void execFuncAlloc(BCFuncAlloc bc) {
        env.mem.setLocal(bc.pointer, bc.offset);
    ...
    private void execLoadAttr(BCLoadDynamic bc) {
        env.stack.push(env.mem.getRel(bc.id));
    }
    ...
    private void execStoreAttr(BCStoreAttr bc) {
        if (bc.obj != -1) {
            env.mem.$THIS = bc.obj;
        }
        env.mem.setRel(bc.id, env.stack.pop());
    }
